...
export const loginUser = async (email, password) => {
  try {
    ...
    console.log("🔗 URL de API:", `${API_BASE_URL}/api/login`)
...
export const logoutApi = async () => {
  try {
    ...
    console.log("🔗 URL:", `${API_BASE_URL}/api/logout`)
...
export const recoverPassword = async (email) => {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json()
      throw new Error(errorData.mensaje || errorData.message || "Error al enviar correo de recuperación")
    }
