...
const DashboardHome = () => {
  ...
  const { selectedConnection, connectionString } = useConnections(user)
  
  // Estado para datos dinámicos por conexión
  const [dbData, setDbData] = useState(null)
  const [loading, setLoading] = useState(false)
  const [currentConnectionString, setCurrentConnectionString] = useState(null)

  // Función para cargar datos según la conexión activa
  const loadConnectionData = async (connection) => {
    if (!connection) return

    setLoading(true)
    setCurrentConnectionString(connection.connection) // Actualizar el connectionString actual
    console.log("📊 Cargando datos para:", connection.name, `(${connection.connection})`)
    ...
    try {
      ...
      const connectionData = mockData[connection.connection] || mockData.conexion1

      setDbData(connectionData)
      console.log("✅ Datos cargados exitosamente para:", connection.name, `(${connection.connection})`)

    } catch (error) {
      console.error("❌ Error cargando datos de conexión:", error)
      setDbData({
        database_name: connection.name || "Sin conexión",
        description: connection.description || "No hay descripción disponible",
        modules: ["Sin módulos"],
        stats: {
          total_users: 0,
          active_projects: 0,
  ...
  useEffect(() => {
    if (selectedConnection) {
      loadConnectionData(selectedConnection)
    }
  }, [selectedConnection])

  // Escuchar cambios de conexión desde cualquier parte de la app
  useEffect(() => {
    const handleConnectionChange = (event) => {
      const newConnection = event.detail
      console.log("🔄 DashboardHome detectó cambio de conexión:", newConnection.name, `(${newConnection.connection})`)
      loadConnectionData(newConnection)
    }
        ...
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Cargando datos de {selectedConnection?.name}...
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="dashboard-home">
      ...
      <motion.div
        ...
        key={`welcome-${selectedConnection?.id}`}
      >
        <div>
          <h2>Bienvenido, {user?.name}! 👋</h2>
          <p>
            Aquí está el resumen de tu compañia: <strong>{dbData?.database_name}</strong>
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Conexión activa: <span className="font-mono text-blue-600 dark:text-blue-400 font-semibold">
              {currentConnectionString || selectedConnection?.connection || "No disponible"}
            </span>
          </p>
        </div>
      ...
      <div className="stats-grid">
        {stats.map((stat, index) => {
          ...
          return (
            <motion.div
              key={`${currentConnectionString}-stat-${index}`}
      ...
      {/* Chart con key para forzar re-render */}
      <div key={`chart-${currentConnectionString}`}>
        <SalesChart connectionString={currentConnectionString} />
      </div>
      ...
      <motion.div
        ...
        key={`db-info-${currentConnectionString}`}
      >
        ...
        <div className="db-modules">
          ...
          <div className="modules-grid">
            {(dbData?.modules || []).map((module, index) => (
              <motion.div
                key={`${currentConnectionString}-module-${index}`}
      ...
      <motion.div
        ...
        key={`activities-${currentConnectionString}`}
      >
